...
import CocktailListPage from './components/CocktailListPage'; // 💡 새로 추가된 리스트 컴포넌트 import
...
type Page =
  'HOME' |
  'DRINK_CATEGORY' |
  'COCKTAIL_INFO_PAGE' | // 이 페이지 ID에서 리스트를 렌더링합니다.
  'TOOLS' |
  'TECHNIQUES' |
  'RELATED' |
  'SPIRIT_LIST_PAGE';
...
// sul.json 구조를 기반으로 타입 정의 (CocktailListPage와 동일해야 함)
interface CocktailData {
  ...
  method_kr: string;
  category: string; // sul.json에 있는 category 필드
}
  ...
  const renderPageContent = () => {
    switch (currentPage) {
      ...
      case 'COCKTAIL_INFO_PAGE':
        // 💡 새로운 칵테일 리스트 페이지를 렌더링하고 데이터를 Props로 전달합니다.
        return <CocktailListPage cocktails={allCocktails} />;
